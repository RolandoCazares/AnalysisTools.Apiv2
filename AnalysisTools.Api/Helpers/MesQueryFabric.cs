...
namespace analysistools.api.Helpers
{
    ...
    public static class MesQueryFabric
    {
        ...
        public static string QueryForProducedAndFilteredFPY(string Producto, DateTime FromDate, DateTime ToDate)
        {
            ...
            return $"SELECT TO_CHAR(t1.MAX_DATE, 'YYYY-MM-DD') AS DATE, t1.PDK_MATERIAL AS MATERIAL, t1.BMT_NAME AS NAME, t1.PRP_VAR AS VAR, t1.RUNID_TYPE AS IDTYPE, COUNT(*) AS CANTIDAD
FROM(
  SELECT AL1.RUNID, AL2.PDK_AUFTR, AL1.RUN_STATE, AL1.RUN_DATE, AL3.PDK_MATERIAL, AL5.BMT_NAME, AL4.PRP_VAR, AL1.RUNID_TYPE, MAX(AL1.RUN_DATE) AS MAX_DATE
  FROM EVAPROD.PD_LFD_RUN AL1, EVAPROD.PD_LFD_AUF AL2, EVAPROD.PD_LFD_MAT AL3, EVAPROD.PD_STM_PRP AL4, EVAPROD.PD_LFD_BMN AL5
  WHERE(AL5.BMT_DAT_ID = AL1.BMT_DAT_ID AND AL2.PRD_SPC_SID = AL1.PRD_SPC_SID AND AL3.PRD_MAT_SID = AL1.PRD_MAT_SID AND AL1.PRP_DATE_ID = AL4.PRP_DATE_ID)
  AND(AL1.RUNID_TYPE LIKE '{product}' AND AL1.RUN_DATE BETWEEN TO_DATE('{fromTestDateString}', 'DD-MON-YYYY HH24:MI:SS') AND TO_DATE('{toTestDateString}', 'DD-MON-YYYY HH24:MI:SS'))
  GROUP BY AL1.RUNID, AL2.PDK_AUFTR, AL1.RUN_STATE, AL1.RUN_DATE, AL3.PDK_MATERIAL, AL5.BMT_NAME, AL4.PRP_VAR, AL1.RUNID_TYPE
) t1
INNER JOIN tabla_ejemplo t2
ON t1.RUNID = t2.SerialNumber AND t1.PDK_MATERIAL = t2.MATERIAL AND t1.BMT_NAME = t2.NAME AND t1.PRP_VAR = t2.VAR AND t1.RUNID_TYPE = t2.IDTYPE AND t1.MAX_DATE = t2.DATE
GROUP BY TO_CHAR(t1.MAX_DATE, 'YYYY-MM-DD'), t1.PDK_MATERIAL, t1.BMT_NAME, t1.PRP_VAR, t1.RUNID_TYPE;
