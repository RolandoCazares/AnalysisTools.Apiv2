...
namespace analysistools.api.Helpers
{
    ...
    public static class MesQueryFabric
    {
        ...
        public static string QueryForProducedAndFilteredFPY(string Producto, DateTime FromDate, DateTime ToDate)
        {
            ...
            //return $"SELECT TO_CHAR(TRUNC(RUN_DATE, 'DD/MM/YYYY'), 'DD/MM/YYYY') AS Fecha, AL3.PDK_MATERIAL AS MATERIAL, AL5.BMT_NAME AS NAME, AL4.PRP_VAR AS VAR, AL1.RUNID_TYPE AS IDTYPE COUNT(*) AS Cantidad FROM FPYDATA WHERE (SerialNumber, VAR, DATE) IN\r\n    (\r\n        SELECT SerialNumber, VAR, MAX(DATE) FROM FPYDATA\r\n        GROUP BY SerialNumber, VAR\r\n    )\r\nGROUP BY\r\n    TO_CHAR(TRUNC(DATE, 'DD/MM/YYYY'), 'DD/MM/YYYY'), MATERIAL, NAME, VAR, IDTYPE\r\nORDER BY\r\n    DAY, MATERIAL, NAME, VAR, IDTYPE"
            //return $"SELECT TO_CHAR(AL1.RUN_DATE, 'YYYY-MM-DD') AS Fecha, AL3.PDK_MATERIAL AS MATERIAL, AL5.BMT_NAM AS NAME, AL4.PRP_VAR AS VAR, AL1.RUNID_TYPE AS IDTYPE, COUNT(*) AS Cantidad FROM ( SELECT AL1.RUNID, AL2.PDK_AUFTR, AL1.RUN_STATE, AL1.RUN_DATE, AL3.PDK_MATERIAL, AL5.BMT_NAME, AL4.PRP_VAR, AL1.RUNID_TYPE FROM EVAPROD.PD_LFD_RUN AL1, EVAPROD.PD_LFD_AUF AL2, EVAPROD.PD_LFD_MAT AL3, EVAPROD.PD_STM_PRP AL4, EVAPROD.PD_LFD_BMN AL5 WHERE (AL5.BMT_DAT_ID=AL1.BMT_DAT_ID AND AL2.PRD_SPC_SID=AL1.PRD_SPC_SID AND AL3.PRD_MAT_SID=AL1.PRD_MAT_SID AND AL1.PRP_DATE_ID=AL4.PRP_DATE_ID) AND (AL1.RUNID_TYPE LIKE '{Producto}' AND AL1.RUN_DATE BETWEEN TO_DATE('{fromDate}', 'dd-mon-yyyy hh24:mi:ss') AND TO_DATE('{toDate}', 'dd-mon-yyyy hh24:mi:ss')) ) GROUP BY TO_CHAR(RUN_DATE, 'YYYY-MM-DD'), PDK_MATERIAL, BMT_NAME, PRP_VAR, RUNID_TYPE";
            //  return $"SELECT TO_CHAR(MAX(AL1.RUN_DATE), 'YYYY-MM-DD') AS Fecha, AL3.PDK_MATERIAL AS MATERIAL, AL5.BMT_NAME AS NAME, AL4.PRP_VAR AS VAR, AL1.RUNID_TYPE AS IDTYPE, COUNT(*) AS Cantidad FROM EVAPROD.PD_LFD_RUN AL1, EVAPROD.PD_LFD_AUF AL2, EVAPROD.PD_LFD_MAT AL3, EVAPROD.PD_STM_PRP AL4, EVAPROD.PD_LFD_BMN AL5 WHERE (AL5.BMT_DAT_ID = AL1.BMT_DAT_ID AND AL2.PRD_SPC_SID = AL1.PRD_SPC_SID AND AL3.PRD_MAT_SID = AL1.PRD_MAT_SID AND AL1.PRP_DATE_ID = AL4.PRP_DATE_ID) AND (AL1.RUNID_TYPE LIKE '{Producto}' AND AL1.RUN_DATE BETWEEN TO_DATE('{fromDate}', 'dd-mon-yyyy hh24:mi:ss') AND TO_DATE('{toDate}', 'dd-mon-yyyy hh24:mi:ss')) GROUP BY TO_CHAR(AL1.RUN_DATE, 'YYYY-MM-DD'), AL3.PDK_MATERIAL, AL5.BMT_NAME, AL4.PRP_VAR, AL1.RUNID_TYPE";
                       return $"SELECT TO_CHAR(t1.MAX_DATE, 'YYYY-MM-DD') AS Fecha, t1.PDK_MATERIAL AS MATERIAL, t1.BMT_NAME AS NAME, t1.PRP_VAR AS VAR, t1.RUNID_TYPE AS IDTYPE, COUNT(*) AS Cantidad FROM (SELECT AL1.RUNID, AL2.PDK_AUFTR, AL1.RUN_STATE, AL1.RUN_DATE, AL3.PDK_MATERIAL, AL5.BMT_NAME, AL4.PRP_VAR, AL1.RUNID_TYPE, MAX(AL1.RUN_DATE) AS MAX_DATE FROM EVAPROD.PD_LFD_RUN AL1, EVAPROD.PD_LFD_AUF AL2, EVAPROD.PD_LFD_MAT AL3, EVAPROD.PD_STM_PRP AL4, EVAPROD.PD_LFD_BMN AL5 WHERE (AL5.BMT_DAT_ID=AL1.BMT_DAT_ID AND AL2.PRD_SPC_SID = AL1.PRD_SPC_SID AND AL3.PRD_MAT_SID = AL1.PRD_MAT_SID AND AL1.PRP_DATE_ID = AL4.PRP_DATE_ID) AND (AL1.RUNID_TYPE LIKE '{Producto}' AND AL1.RUN_DATE BETWEEN TO_DATE('{fromDate}', 'dd-MMM-yyyy HH:mm:ss') AND TO_DATE('{toDate}', 'dd-MMM-yyyy HH:mm:ss')) GROUP BY AL1.RUNID, AL2.PDK_AUFTR, AL1.RUN_STATE, AL1.RUN_DATE, AL3.PDK_MATERIAL, AL5.BMT_NAME, AL4.PRP_VAR, AL1.RUNID_TYPE) t1 INNER JOIN tabla_ejemplo t2 ON t1.RUNID = t2.SerialNumber AND t1.PDK_MATERIAL = t2.MATERIAL AND t1.BMT_NAME = t2.NAME AND t1.PRP_VAR = t2.VAR AND t1.RUNID_TYPE = t2.IDTYPE AND t1.MAX_DATE = t2.DATE GROUP BY TO_CHAR(t1.MAX_DATE, 'YYYY-MM-DD'), t1.PDK_MATERIAL, t1.BMT_NAME, t1.PRP_VAR, t1.RUNID_TYPE";
                     //return $"SELECT TO_CHAR(t1.MAX_DATE, 'YYYY-MM-DD') AS Fecha, t1.PDK_MATERIAL AS MATERIAL, t1.BMT_NAME AS NAME, t1.PRP_VAR AS VAR, t1.RUNID_TYPE AS IDTYPE, COUNT(*) AS Cantidad FROM (SELECT AL1.RUNID AS SerialNumber, AL2.PDK_AUFTR, AL1.RUN_STATE, AL1.RUN_DATE, AL3.PDK_MATERIAL, AL5.BMT_NAME, AL4.PRP_VAR, AL1.RUNID_TYPE, MAX(AL1.RUN_DATE) AS MAX_DATE FROM EVAPROD.PD_LFD_RUN AL1, EVAPROD.PD_LFD_AUF AL2, EVAPROD.PD_LFD_MAT AL3, EVAPROD.PD_STM_PRP AL4, EVAPROD.PD_LFD_BMN AL5 WHERE (AL5.BMT_DAT_ID=AL1.BMT_DAT_ID AND AL2.PRD_SPC_SID = AL1.PRD_SPC_SID AND AL3.PRD_MAT_SID = AL1.PRD_MAT_SID AND AL1.PRP_DATE_ID = AL4.PRP_DATE_ID) AND (AL1.RUNID_TYPE LIKE '{Producto}' AND AL1.RUN_DATE BETWEEN TO_DATE('{fromDate}', 'dd-MMM-yyyy HH:mm:ss') AND TO_DATE('{toDate}', 'dd-MMM-yyyy HH:mm:ss')) GROUP BY AL1.RUNID, AL2.PDK_AUFTR, AL1.RUN_STATE, AL1.RUN_DATE, AL3.PDK_MATERIAL, AL5.BMT_NAME, AL4.PRP_VAR, AL1.RUNID_TYPE) t1 INNER JOIN tabla_ejemplo t2 ON t1.SerialNumber = t2.SerialNumber AND t1.PDK_MATERIAL = t2.MATERIAL AND t1.BMT_NAME = t2.NAME AND t1.PRP_VAR = t2.VAR AND t1.RUNID_TYPE = t2.IDTYPE AND t1.MAX_DATE = t2.DATE GROUP BY TO_CHAR(t1.MAX_DATE, 'YYYY-MM-DD'), t1.PDK_MATERIAL, t1.BMT_NAME, t1.PRP_VAR, t1.RUNID_TYPE";

            //

            //return $"SELECT TO_CHAR(t1.MAX_DATE, 'YYYY-MM-DD') AS Fecha, t1.PDK_MATERIAL AS MATERIAL, t1.BMT_NAME AS NAME, t1.PRP_VAR AS VAR, t1.RUNID_TYPE AS IDTYPE, COUNT(*) AS Cantidad FROM (SELECT AL1.RUNID AS SerialNumber, AL2.PDK_AUFTR, AL1.RUN_STATE, AL1.RUN_DATE, AL3.PDK_MATERIAL, AL5.BMT_NAME, AL4.PRP_VAR, AL1.RUNID_TYPE, MAX(AL1.RUN_DATE) AS MAX_DATE FROM EVAPROD.PD_LFD_RUN AL1, EVAPROD.PD_LFD_AUF AL2, EVAPROD.PD_LFD_MAT AL3, EVAPROD.PD_STM_PRP AL4, EVAPROD.PD_LFD_BMN AL5 WHERE (AL5.BMT_DAT_ID=AL1.BMT_DAT_ID AND AL2.PRD_SPC_SID = AL1.PRD_SPC_SID AND AL3.PRD_MAT_SID = AL1.PRD_MAT_SID AND AL1.PRP_DATE_ID = AL4.PRP_DATE_ID) AND (AL1.RUNID_TYPE LIKE '{Producto}' AND AL1.RUN_DATE BETWEEN TO_DATE('{fromDate}', 'dd-MMM-yyyy HH:mm:ss') AND TO_DATE('{toDate}', 'dd-MMM-yyyy HH:mm:ss')) GROUP BY AL1.RUNID, AL2.PDK_AUFTR, AL1.RUN_STATE, AL1.RUN_DATE, AL3.PDK_MATERIAL, AL5.BMT_NAME, AL4.PRP_VAR, AL1.RUNID_TYPE) t1 INNER JOIN tabla_ejemplo t2 ON t1.SerialNumber = t2.SerialNumber AND t1.PDK_MATERIAL = t2.MATERIAL AND t1.BMT_NAME = t2.NAME AND t1.PRP_VAR = t2.VAR AND t1.RUNID_TYPE = t2.IDTYPE AND t1.MAX_DATE = t2.DATE GROUP BY TO_CHAR(t1.MAX_DATE, 'YYYY-MM-DD'), t1.PDK_MATERIAL, t1.BMT_NAME, t1.PRP_VAR, t1.RUNID_TYPE";
            //SELECT AL1.RUNID, AL2.PDK_AUFTR, AL1.RUN_STATE, AL1.RUN_DATE, AL3.PDK_MATERIAL, AL5.BMT_NAME, AL4.PRP_VAR, AL1.RUNID_TYPE                                FROM EVAPROD.PD_LFD_RUN AL1, EVAPROD.PD_LFD_AUF AL2, EVAPROD.PD_LFD_MAT AL3, EVAPROD.PD_STM_PRP AL4, EVAPROD.PD_LFD_BMN AL5 WHERE (AL5.BMT_DAT_ID=AL1.BMT_DAT_ID AND AL2.PRD_SPC_SID = AL1.PRD_SPC_SID AND AL3.PRD_MAT_SID = AL1.PRD_MAT_SID AND AL1.PRP_DATE_ID = AL4.PRP_DATE_ID) AND (AL1.RUNID_TYPE LIKE '{Producto}' AND AL1.RUN_DATE BETWEEN TO_DATE('{fromDate}', 'dd-mon-yyyy hh24:mi:ss') AND TO_DATE('{toDate}', 'dd-mon-yyyy hh24:mi:ss'))";
